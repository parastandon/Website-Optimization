...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
	  ...
	  var my = determineDx(Contain[0], size);
	  var newwidth = (Contain[0].offsetWidth + my) + 'px';
    for (var s = 0; s< Contain.length; s++) {
      Contain[s].style.width = newwidth;
    }
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  for(var k=0; k<49; k++) {
    ...
    elem.basicLeft = (k % d) * y;
    elem.style.top = (Math.floor(k / d) * y) + 'px';
